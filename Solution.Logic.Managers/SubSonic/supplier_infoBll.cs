...
namespace Solution.Logic.Managers {
	...
	/// supplier_info表逻辑类
	/// </summary>
	public partial class supplier_infoBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_supplier_info";
        private const string const_CacheKey_Date = "Cache_supplier_info_Date";

		#region 单例模式
		//定义单例实体
		private static supplier_infoBll _supplier_infoBll = null;

		/// <summary>
		/// 获取本逻辑类单例
		/// </summary>
		/// <returns></returns>
		public static supplier_infoBll GetInstence() {
			if (_supplier_infoBll == null) {
				_supplier_infoBll = new supplier_infoBll();
			}
			return _supplier_infoBll;
		}
		#endregion
		...
		#region 从IIS缓存中获取supplier_info表记录
		/// <summary>
        /// 从IIS缓存中获取supplier_info表记录
        /// </summary>
	    /// <param name="isCache">是否从缓存中读取</param>
        public IList<DataAccess.Model.supplier_info> GetList(bool isCache = true)
        {
			try
			{
				//判断是否使用缓存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.supplier_info>)obj;
					}
				}
				else
				{
					//定义临时实体集
					IList<DataAccess.Model.supplier_info> list = null;

					//获取全表缓存加载条件表达式
					var exp = GetExpression<supplier_info>();
                    //如果条件为空，则查询全表所有记录
					if (exp == null)
					{
						//从数据库中获取所有记录
						var all = supplier_info.All();
                        list = all == null ? null : Transform(all.ToList());
					}
					else
					{
                        //从数据库中查询出指定条件的记录，并转换为指定实体集
						var all = supplier_info.Find(exp);
            ...
            {
                //记录日志
                CommonBll.WriteLog("从IIS缓存中获取supplier_info表记录时出现异常", e);
			}
        ...
        /// <param name="isCache">是否从缓存中读取</param>
		/// <returns>DataAccess.Model.supplier_info</returns>
        public DataAccess.Model.supplier_info GetModel(long id, bool isCache = true)
        {
            //判断是否使用缓存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //从数据库中直接读取
                var model = supplier_info.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主键Id</param>
		/// <returns>DataAccess.Model.supplier_info</returns>
        public DataAccess.Model.supplier_info GetModelForCache(long id)
        {
			try
			{
				//从缓存中读取指定Id记录
				...
				if (model == null){
					//从数据库中读取
					var tem = supplier_info.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.supplier_info</returns>
        public DataAccess.Model.supplier_info GetModelForCache(string conditionColName, object value)
        {
		try
            {
                //从缓存中获取List
                var list = GetList();
                DataAccess.Model.supplier_info model = null;
                Expression<Func<supplier_info, bool>> expression = null;
                ...
                switch (conditionColName)
                {
					case "Id" :
                        ...
                        break;
					case "supplier_code" :
						model = list.SingleOrDefault(x => x.supplier_code == (string)value);
                        expression = x => x.supplier_code == (string)value;
                        break;
					case "supplier_name" :
						model = list.SingleOrDefault(x => x.supplier_name == (string)value);
                        expression = x => x.supplier_name == (string)value;
                        break;
					case "supplier_connecter" :
						model = list.SingleOrDefault(x => x.supplier_connecter == (string)value);
                        expression = x => x.supplier_connecter == (string)value;
                        break;
					case "supplier_phone" :
						model = list.SingleOrDefault(x => x.supplier_phone == (string)value);
                        expression = x => x.supplier_phone == (string)value;
                        break;
					case "supplier_address" :
						model = list.SingleOrDefault(x => x.supplier_address == (string)value);
                        expression = x => x.supplier_address == (string)value;
                        break;
					case "supplier_distance" :
						model = list.SingleOrDefault(x => x.supplier_distance == (string)value);
                        expression = x => x.supplier_distance == (string)value;
                        break;
					case "supplier_product" :
						model = list.SingleOrDefault(x => x.supplier_product == (string)value);
                        expression = x => x.supplier_product == (string)value;
                        break;
					case "supplier_account" :
						model = list.SingleOrDefault(x => x.supplier_account == (string)value);
                        expression = x => x.supplier_account == (string)value;
                        break;
					case "supplier_type" :
						model = list.SingleOrDefault(x => x.supplier_type == (string)value);
                        expression = x => x.supplier_type == (string)value;
                        break;
                ...
                {
                    //从数据库中读取
                    var tem = supplier_info.SingleOrDefault(expression);
                    if (tem == null)
                    {
                        return null;
                    }
        ...
        /// <returns>DataAccess.Model.supplier_info</returns>
        public DataAccess.Model.supplier_info GetModelForCache(Expression<Func<DataAccess.Model.supplier_info, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.supplier_info model)
        {
            //从缓存中读取记录列表
            var list = GetList();
            //从缓存中删除记录
        ...
        /// </summary>
        /// <param name="model">记录实体</param>
        public void SetModelForCache(supplier_info model)
        {
		...
		/// <summary>
        /// 按条件删除IIS缓存中supplier_info表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.supplier_info, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
		...
		#region 实体转换
		/// <summary>
		/// 将supplier_info记录实体（SubSonic实体）转换为普通的实体（DataAccess.Model.supplier_info）
		/// </summary>
        /// <param name="model">SubSonic插件生成的实体</param>
		/// <returns>DataAccess.Model.supplier_info</returns>
		public DataAccess.Model.supplier_info Transform(supplier_info model)
        {			
			if (model == null) 
				return null;

            return new DataAccess.Model.supplier_info
            {
                Id = model.Id,
                supplier_code = model.supplier_code,
                supplier_name = model.supplier_name,
                supplier_connecter = model.supplier_connecter,
                supplier_phone = model.supplier_phone,
                supplier_address = model.supplier_address,
                supplier_distance = model.supplier_distance,
                supplier_product = model.supplier_product,
                supplier_account = model.supplier_account,
                supplier_type = model.supplier_type,
            };
        }

		/// <summary>
		/// 将supplier_info记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.supplier_info）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.supplier_info> Transform(IList<supplier_info> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.supplier_info>();
		...
		/// <summary>
		/// 将supplier_info记录实体由普通的实体（DataAccess.Model.supplier_info）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.supplier_info）</param>
		/// <returns>supplier_info</returns>
		public supplier_info Transform(DataAccess.Model.supplier_info model)
        {
			if (model == null) 
				return null;
		...
		/// <summary>
		/// 将supplier_info记录实体由普通实体集（DataAccess.Model.supplier_info）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.supplier_info）</param>
        public IList<supplier_info> Transform(IList<DataAccess.Model.supplier_info> sourceList)
        {
			//创建List容器
            var list = new List<supplier_info>();
		...
		public void SetModelValue(DataAccess.Model.supplier_info model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.supplier_info model, string colName, object value)
		{
            ...
            switch (colName)
            {
				case "Id" :
                    ...
                    break;
				case "supplier_code" :
					model.supplier_code = (string)value;
                    break;
				case "supplier_name" :
					model.supplier_name = (string)value;
                    break;
				case "supplier_connecter" :
					model.supplier_connecter = (string)value;
                    break;
				case "supplier_phone" :
					model.supplier_phone = (string)value;
                    break;
				case "supplier_address" :
					model.supplier_address = (string)value;
                    break;
				case "supplier_distance" :
					model.supplier_distance = (string)value;
                    break;
				case "supplier_product" :
					model.supplier_product = (string)value;
                    break;
				case "supplier_account" :
					model.supplier_account = (string)value;
                    break;
				case "supplier_type" :
					model.supplier_type = (string)value;
                    break;
            }
		}
		...
		#region 获取supplier_info表记录总数
        /// <summary>
        /// 获取supplier_info表记录总数
        /// </summary>
		...
		/// <summary>
		/// 获取supplier_info表记录总数——从数据库中查询
		/// </summary>
        /// <param name="wheres">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionHelper.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<supplier_info>(wheres);

		}

		/// <summary>
		/// 获取supplier_info表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<supplier_info, bool>> expression) {
            return new Select().From<supplier_info>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.supplier_info> Find(Expression<Func<DataAccess.Model.supplier_info, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        public bool Exist(int id)
        {
            ...
            //从数据库中查找
            return supplier_info.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.supplier_info, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionHelper.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<supplier_info>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取supplier_info表记录时出现异常", e);
		...
		#region 添加与编辑supplier_info表记录
		/// <summary>
		/// 添加与编辑supplier_info记录
		/// </summary>
	    /// <param name="page">当前页面指针</param>
		/// <param name="model">supplier_info表实体</param>
        /// <param name="content">更新说明</param>
        /// <param name="isCache">是否更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Save(Page page, supplier_info model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				//保存
				model.Save();
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "编辑") + "supplier_info记录成功，ID为【" + model.Id + "】";
					}
				...
				var result = "执行supplier_infoBll.Save()函数出错！";
		...
		{
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", supplier_infoTable.TableName,  supplier_infoTable.Id, id);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了supplier_info表id为【" + id + "】的记录！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", supplier_infoTable.TableName,  supplier_infoTable.Id, str);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了supplier_info表id为【" + str + "】的记录！");
			}
		}

		/// <summary>
        /// 删除supplier_info表记录——如果使用了缓存，删除成功后会清空本表的所有缓存记录，然后重新加载进缓存
        /// </summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="expression">条件语句</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Delete(Page page, Expression<Func<supplier_info, bool>> expression, bool isAddUseLog = true)
        {
			//执行删除
			supplier_info.Delete(expression);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了supplier_info表记录！");
			}
        }

		/// <summary>
        /// 删除supplier_info表所有记录
        /// </summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="isAddUseLog">是否添加用户操作日志</param>
        ...
        {
            //设置Sql语句
            var sql = string.Format("delete from {0}", supplier_infoTable.TableName);
            ...
            {
                //添加用户操作记录
                UseLogBll.GetInstence().Save(page, "{0}删除了supplier_info表所有记录！");
            }
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "supplier_info", sortName, "Id"))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}更新了supplier_info表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "supplier_info", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}对supplier_info表进行了自动排序操作！");
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.supplier_info, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<supplier_info>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.supplier_info model, string colName)
		{
			...
			switch (colName)
			{
				...
				case "supplier_connecter" :
					return model.supplier_connecter;
				case "supplier_phone" :
					return model.supplier_phone;
				case "supplier_address" :
					return model.supplier_address;
				case "supplier_distance" :
					return model.supplier_distance;
				case "supplier_product" :
					return model.supplier_product;
				case "supplier_account" :
					return model.supplier_account;
				case "supplier_type" :
					return model.supplier_type;
			}
		...
		#region 更新supplier_info表指定字段值
		/// <summary>更新supplier_info表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionHelper.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<supplier_info>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了supplier_info表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新supplier_info表指定主键Id的字段值
		/// <summary>更新supplier_info表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了supplier_info表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionHelper.SqlqueryCondition(ConstraintType.And, supplier_infoTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新supplier_info表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了supplier_info表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新supplier_info表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了supplier_info表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
