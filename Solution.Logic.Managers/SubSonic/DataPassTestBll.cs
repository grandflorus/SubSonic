...
namespace Solution.Logic.Managers {
	...
	/// DataPassTest表逻辑类
	/// </summary>
	public partial class DataPassTestBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_DataPassTest";
        private const string const_CacheKey_Date = "Cache_DataPassTest_Date";

		#region 单例模式
		//定义单例实体
		private static DataPassTestBll _DataPassTestBll = null;

		/// <summary>
		/// 获取本逻辑类单例
		/// </summary>
		/// <returns></returns>
		public static DataPassTestBll GetInstence() {
			if (_DataPassTestBll == null) {
				_DataPassTestBll = new DataPassTestBll();
			}
			return _DataPassTestBll;
		}
		#endregion
		...
		#region 从IIS缓存中获取DataPassTest表记录
		/// <summary>
        /// 从IIS缓存中获取DataPassTest表记录
        /// </summary>
	    /// <param name="isCache">是否从缓存中读取</param>
        public IList<DataAccess.Model.DataPassTest> GetList(bool isCache = true)
        {
			try
			{
				//判断是否使用缓存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.DataPassTest>)obj;
					}
				}
				else
				{
					//定义临时实体集
					IList<DataAccess.Model.DataPassTest> list = null;

					//获取全表缓存加载条件表达式
					var exp = GetExpression<DataPassTest>();
                    //如果条件为空，则查询全表所有记录
					if (exp == null)
					{
						//从数据库中获取所有记录
						var all = DataPassTest.All();
                        list = all == null ? null : Transform(all.ToList());
					}
					else
					{
                        //从数据库中查询出指定条件的记录，并转换为指定实体集
						var all = DataPassTest.Find(exp);
            ...
            {
                //记录日志
                CommonBll.WriteLog("从IIS缓存中获取DataPassTest表记录时出现异常", e);
			}
        ...
        /// <param name="isCache">是否从缓存中读取</param>
		/// <returns>DataAccess.Model.DataPassTest</returns>
        public DataAccess.Model.DataPassTest GetModel(long id, bool isCache = true)
        {
            //判断是否使用缓存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //从数据库中直接读取
                var model = DataPassTest.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主键Id</param>
		/// <returns>DataAccess.Model.DataPassTest</returns>
        public DataAccess.Model.DataPassTest GetModelForCache(long id)
        {
			try
			{
				//从缓存中读取指定Id记录
				...
				if (model == null){
					//从数据库中读取
					var tem = DataPassTest.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.DataPassTest</returns>
        public DataAccess.Model.DataPassTest GetModelForCache(string conditionColName, object value)
        {
		try
            {
                //从缓存中获取List
                var list = GetList();
                DataAccess.Model.DataPassTest model = null;
                Expression<Func<DataPassTest, bool>> expression = null;
                ...
                switch (conditionColName)
                {
					case "Id" :
                        ...
                        break;
					case "a" :
						model = list.SingleOrDefault(x => x.a == (string)value);
                        expression = x => x.a == (string)value;
                        break;
					case "b" :
						model = list.SingleOrDefault(x => x.b == (string)value);
                        expression = x => x.b == (string)value;
                        break;
					case "c" :
						model = list.SingleOrDefault(x => x.c == (string)value);
                        expression = x => x.c == (string)value;
                        break;
					case "d" :
						model = list.SingleOrDefault(x => x.d == (string)value);
                        expression = x => x.d == (string)value;
                        break;
                ...
                {
                    //从数据库中读取
                    var tem = DataPassTest.SingleOrDefault(expression);
                    if (tem == null)
                    {
                        return null;
                    }
        ...
        /// <returns>DataAccess.Model.DataPassTest</returns>
        public DataAccess.Model.DataPassTest GetModelForCache(Expression<Func<DataAccess.Model.DataPassTest, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.DataPassTest model)
        {
            //从缓存中读取记录列表
            var list = GetList();
            //从缓存中删除记录
        ...
        /// </summary>
        /// <param name="model">记录实体</param>
        public void SetModelForCache(DataPassTest model)
        {
		...
		/// <summary>
        /// 按条件删除IIS缓存中DataPassTest表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.DataPassTest, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
		...
		#region 实体转换
		/// <summary>
		/// 将DataPassTest记录实体（SubSonic实体）转换为普通的实体（DataAccess.Model.DataPassTest）
		/// </summary>
        /// <param name="model">SubSonic插件生成的实体</param>
		/// <returns>DataAccess.Model.DataPassTest</returns>
		public DataAccess.Model.DataPassTest Transform(DataPassTest model)
        {			
			if (model == null) 
				return null;

            return new DataAccess.Model.DataPassTest
            {
                Id = model.Id,
                a = model.a,
                b = model.b,
		...
		/// <summary>
		/// 将DataPassTest记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.DataPassTest）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.DataPassTest> Transform(IList<DataPassTest> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.DataPassTest>();
		...
		/// <summary>
		/// 将DataPassTest记录实体由普通的实体（DataAccess.Model.DataPassTest）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.DataPassTest）</param>
		/// <returns>DataPassTest</returns>
		public DataPassTest Transform(DataAccess.Model.DataPassTest model)
        {
			if (model == null) 
				return null;

            return new DataPassTest
            {
                Id = model.Id,
                a = model.a,
                b = model.b,
		...
		/// <summary>
		/// 将DataPassTest记录实体由普通实体集（DataAccess.Model.DataPassTest）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.DataPassTest）</param>
        public IList<DataPassTest> Transform(IList<DataAccess.Model.DataPassTest> sourceList)
        {
			//创建List容器
            var list = new List<DataPassTest>();
		...
		public void SetModelValue(DataAccess.Model.DataPassTest model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.DataPassTest model, string colName, object value)
		{
            ...
            switch (colName)
            {
				...
				case "a" :
                    ...
                    break;
				case "b" :
					model.b = (string)value;
                    break;
				case "c" :
					model.c = (string)value;
                    break;
				case "d" :
					model.d = (string)value;
                    break;
            }
		}
		...
		/// <summary>
		/// 获取DataPassTest表记录总数——从数据库中查询
		/// </summary>
        /// <param name="wheres">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionHelper.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<DataPassTest>(wheres);

		}

		/// <summary>
		/// 获取DataPassTest表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<DataPassTest, bool>> expression) {
            return new Select().From<DataPassTest>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.DataPassTest> Find(Expression<Func<DataAccess.Model.DataPassTest, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        public bool Exist(int id)
        {
            ...
            //从数据库中查找
            return DataPassTest.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.DataPassTest, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionHelper.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<DataPassTest>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取DataPassTest表记录时出现异常", e);
		...
		#region 添加与编辑DataPassTest表记录
		/// <summary>
		/// 添加与编辑DataPassTest记录
		/// </summary>
	    /// <param name="page">当前页面指针</param>
		/// <param name="model">DataPassTest表实体</param>
        /// <param name="content">更新说明</param>
        /// <param name="isCache">是否更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Save(Page page, DataPassTest model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				//保存
				model.Save();
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "编辑") + "DataPassTest记录成功，ID为【" + model.Id + "】";
					}
				...
				var result = "执行DataPassTestBll.Save()函数出错！";
		...
		{
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", DataPassTestTable.TableName,  DataPassTestTable.Id, id);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了DataPassTest表id为【" + id + "】的记录！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", DataPassTestTable.TableName,  DataPassTestTable.Id, str);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了DataPassTest表id为【" + str + "】的记录！");
			}
		}

		/// <summary>
        /// 删除DataPassTest表记录——如果使用了缓存，删除成功后会清空本表的所有缓存记录，然后重新加载进缓存
        /// </summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="expression">条件语句</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Delete(Page page, Expression<Func<DataPassTest, bool>> expression, bool isAddUseLog = true)
        {
			//执行删除
			DataPassTest.Delete(expression);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了DataPassTest表记录！");
			}
        }
        ...
        {
            //设置Sql语句
            var sql = string.Format("delete from {0}", DataPassTestTable.TableName);
            ...
            {
                //添加用户操作记录
                UseLogBll.GetInstence().Save(page, "{0}删除了DataPassTest表所有记录！");
            }
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "DataPassTest", sortName, "Id"))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}更新了DataPassTest表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "DataPassTest", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}对DataPassTest表进行了自动排序操作！");
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.DataPassTest, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<DataPassTest>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.DataPassTest model, string colName)
		{
		...
		#region 更新DataPassTest表指定字段值
		/// <summary>更新DataPassTest表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionHelper.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<DataPassTest>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了DataPassTest表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新DataPassTest表指定主键Id的字段值
		/// <summary>更新DataPassTest表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了DataPassTest表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionHelper.SqlqueryCondition(ConstraintType.And, DataPassTestTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新DataPassTest表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了DataPassTest表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新DataPassTest表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了DataPassTest表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
