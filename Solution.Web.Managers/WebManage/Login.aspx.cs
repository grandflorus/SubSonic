...
namespace Solution.Web.Managers.WebManage
{
    public partial class Login : System.Web.UI.Page
    {
        ...
        protected void BtnLogin_Click(object sender, EventArgs e)
        {
            ...
            {
                //将在线实体保存到数据库的在线表中
                //因为存储到缓存中的实体必须为简化版的实体，而可以直接存储到数据库的是SubSoinc3.0原生模板生成的实体，所以这里要进行转换，且用原生实体保存成功后，可以得到记录的Id
                var model = OnlineUsersBll.GetInstence().Transform(onlineUser);
                OnlineUsersBll.GetInstence().Save(this, model, null, true, false);
                //将当前记录在在线表中的Id赋给简化版实体，以便能存储到缓存中，方便后面对数据库记录的操作
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        //删除数据库记录与缓存记录
                        OnlineUsersBll.GetInstence().Delete(this, x => x.Id == onlineModel.Id, false);
                    }
                    //移除在线缓存中的记录
